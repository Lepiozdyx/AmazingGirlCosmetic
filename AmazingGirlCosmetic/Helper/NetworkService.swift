...
class NetworkService: ObservableObject {
    @Published private(set) var petURL: URL?
    
    static let initURL = URL(string: "https://amazgirlcosm.pro/server")!
    
    private let storage: UserDefaults
    private var didSaveURL = false
    private let requestTimeout: TimeInterval = 7.0
...
struct WKWebViewManager: UIViewRepresentable {
    class Coordinator: NSObject, WKNavigationDelegate {
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            ...
            if finalURL != NetworkService.initURL {
